        ...
        for elem in self.urls:          
            try:
                ...
                if os.path.exists(file_name):
                    print 'File name %s already exists. Overwrite?' %(file_name)
                    input = raw_input('[y/n]')
                    if not input.lower() == 'y':
                        print 'Skipping...'
                else:
...
def main():
    ...
    # Change the url to blog format to scrape all the urls.
    pattern = re.match(r'http://imgur.com/a/[\w\d]+', album_url)   
    album_url = album_url[:pattern.end()] + '/layout/blog'
